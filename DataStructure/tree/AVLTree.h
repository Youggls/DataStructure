...
namespace dataStructure {
	...
	private:
		...
		AVLTreeNode<T>* getFirst(AVLTreeNode<T>* t) {
		...
		AVLTreeNode<T>* getLast(AVLTreeNode<T>* t) {
			if (t == NULL) return NULL;
			while (t->right) {
				t = t->right;
			}
		...
		AVLTreeNode<T>* getNext(AVLTreeNode<T>* t) {
			if (t == NULL) return t;
			...
			if (t->right) {
				...
				while (true) {
					AVLTreeNode<T>* last = t;
					t = t->ancestor;
					if (t == NULL) break;
					if (t->left == last) break;
				}
			}

			return t;
		}
		AVLTreeNode<T>* getPrev(AVLTreeNode<T>* t) {
			if (t == NULL) return t;
		...
		void replaceChild(AVLTreeNode<T>* oldNode, AVLTreeNode<T>* newNode, AVLTreeNode<T>* ancestor) {
			if (ancestor) {
				if (ancestor->left == oldNode)
					ancestor->left = newNode;
				else ancestor->right = newNode;
			}
			else root = newNode;
		}
		void replaceNode(AVLTreeNode<T>* node, AVLTreeNode<T>* newNode) {
			AVLTreeNode<T>* ancestor = node->ancestor;
			replaceChild(node, newNode, ancestor);

			if (node->left) node->left->ancestor = newNode;
			if (node->right) node->right->ancestor = newNode;

			newNode->left = node->left;
			newNode->right = node->right;
			newNode->ancestor = node->ancestor;
		...
		AVLTreeNode<T>* rightRotation(AVLTreeNode<T>* node) {
			...
			if (left->right) left->right->ancestor = node;
			left->right = node;
			left->ancestor = ancestor;
			replaceChild(node, left, ancestor);
			node->ancestor = left;
			return left;
		}
		AVLTreeNode<T>* leftRotation(AVLTreeNode<T>* node) {
			...
			if (right->left) right->left->ancestor = node;
			right->left = node;
			right->ancestor = ancestor;
			replaceChild(node, right, ancestor);
			node->ancestor = right;
			return right;
		}

		int _getHeigt(AVLTreeNode<T>* t) {
			if (t == NULL) return 0;
		...
		AVLTreeNode<T>* fixLeft(AVLTreeNode<T>* node) {
			AVLTreeNode<T>* right = node->right;
			int rlHeight = _getHeigt(right->left);
			int rrHeight = _getHeigt(right->right);

			if (rlHeight > rrHeight) {
				right = rightRotation(right);
				updateHeight(right->right);
				updateHeight(right);
			}
		...
		AVLTreeNode<T>* fixRight(AVLTreeNode<T>* node) {
			AVLTreeNode<T>* left = node->left;
			int llHeight = _getHeigt(left->left);
			int lrHeight = _getHeigt(left->right);
		...
		void insert(AVLTreeNode<T>* node) {
			node->height = 1;
			for (node = node->ancestor; node; node = node->ancestor) {
				int hl = _getHeigt(node->left);
				int hr = _getHeigt(node->right);
		...
		void erase(AVLTreeNode<T>* node) {
			...
			if (node->left && node->right) {
				...
				replaceChild(node, child, ancestor);
				if (node->ancestor == old) {
					ancestor = node;
				}
	...
	void AVLTree<T>::insert(T& d) {
		AVLTreeNode<T>* node = new AVLTreeNode<T>(d);
		if (root == NULL) {
			root = node;
		}
		else {
			AVLTreeNode<T>* t = root;
			AVLTreeNode<T>* insertPos = NULL;
			while (t && t->dat != d) {
				if (t->dat > d) {
					insertPos = t;
					t = t->left;
	...
	template <class T>
	void AVLTree<T>::erase(T& d) {
		AVLTreeNode<T>* node = new AVLTreeNode<T>(d);
		AVLTreeNode<T>* t = root;
		AVLTreeNode<T>* erasePos = NULL;
		if (root == NULL) {
			return;
		}
		else {
